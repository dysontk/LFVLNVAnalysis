...
C     
C     Process: d d~ > z z u u~ WEIGHTED<=6 @2
C     *   Decay: z > e+ e- WEIGHTED<=2
C     *   Decay: z > mu+ mu- WEIGHTED<=2
C     Process: d d~ > z z c c~ WEIGHTED<=6 @2
C     *   Decay: z > e+ e- WEIGHTED<=2
C     *   Decay: z > mu+ mu- WEIGHTED<=2
C     Process: s s~ > z z u u~ WEIGHTED<=6 @2
C     *   Decay: z > e+ e- WEIGHTED<=2
C     *   Decay: z > mu+ mu- WEIGHTED<=2
C     Process: s s~ > z z c c~ WEIGHTED<=6 @2
C     *   Decay: z > e+ e- WEIGHTED<=2
C     *   Decay: z > mu+ mu- WEIGHTED<=2
C     
      USE DISCRETESAMPLER
            ...
            DO I=1,NCOMB
              IF(INIT_MODE) THEN
                IF (DABS(TS(I)).GT.ANS*LIMHEL/NCOMB) THEN
                  PRINT *, 'Matrix Element/Good Helicity: 24 ', I,
     $              'IMIRROR', IMIRROR
                ENDIF
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
     ...
     $       //' zero but not the total matrix-element'
      ...
      REAL*8 FUNCTION MATRIX24(P,NHEL,IC, IHEL)
C     
C     Generated by MadGraph5_aMC@NLO v. 3.5.3, 2023-12-23
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      ...
      LOGICAL ZEROAMP_24(NCOMB,NGRAPHS)
      COMMON/TO_ZEROAMP_24/ZEROAMP_24
...
C     Amplitude(s) for diagram number 1
      ...
      CALL FFV1P0_3(W(1,1),W(1,2),GC_11,ZERO, FK_ZERO,W(1,10))
...
C     Amplitude(s) for diagram number 8
      ...
      CALL FFV1P0_3(W(1,4),W(1,3),GC_11,ZERO, FK_ZERO,W(1,13))
...
C     Amplitude(s) for diagram number 20
      CALL FFV1_0(W(1,1),W(1,9),W(1,3),GC_11,AMP(20))
...
C     JAMPs contributing to orders ALL_ORDERS=1
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ...
          DO N = 1, NAMPSO
            ...
            MATRIX24 = MATRIX24 + ZTEMP*DCONJG(JAMP(I,N))
      ...
      DO I=1, NGRAPHS
        ALL_FALSE = .TRUE.
        DO J=1,NCOMB
          IF (.NOT.ZEROAMP_24(J, I)) THEN
            ALL_FALSE = .FALSE.
            EXIT
          ENDIF
        ENDDO
        IF (ALL_FALSE) THEN
          WRITE(*,*) 'Amplitude/ZEROAMP:', 24, I
        ELSE
          DO J=1,NCOMB
            IF (ZEROAMP_24(J, I)) THEN
              WRITE(*,*) 'HEL/ZEROAMP:', 24, J  , I
            ENDIF
          ENDDO
        ENDIF
      ENDDO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION SQSOINDEX24(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/AMPSPLITORDERS24/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER SOINDEX_FOR_SQUARED_ORDERS24
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   +AMPSPLITORDERS(ORDERINDEXB,I)
      ENDDO
      SQSOINDEX24=SOINDEX_FOR_SQUARED_ORDERS24(SQORDERS)
      END

      INTEGER FUNCTION SOINDEX_FOR_SQUARED_ORDERS24(ORDERS)
C     
...
C     
      ...
      COMMON/SQPLITORDERS24/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        SOINDEX_FOR_SQUARED_ORDERS24 = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'SOINDEX_FOR_SQUARED_ORDERS24'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE GET_NSQSO_BORN24(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE GET_SQUARED_ORDERS_FOR_SOINDEX24(SOINDEX,ORDERS)
C     
...
C     
     ...
     $ //' GET_SQUARED_ORDERS_FOR_SOINDEX24'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE GET_ORDERS_FOR_AMPSOINDEX24(SOINDEX,ORDERS)
C     
...
C      completeness
      INTEGER FUNCTION SOINDEX_FOR_AMPORDERS24(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        SOINDEX_FOR_AMPORDERS24 = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function SOINDEX_FOR_AMPORDERS24'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
